...
class GaussianPredictor(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if cfg.model.gaussian_rendering:
            ...
            render_camera =self.render_images(inputs, outputs) 
            outputs['camera_list']= render_camera 
        return outputs
    ...
    # project
    def get_valid_project(xyz, c2w, K, image_wh):
        """
            image_wh: (w, h)
            xyz: (N, 3)
            c2w: (4, 4)
        ...
        w, h = image_wh
        N, _ = xyz.shape
        homo_xyz = torch.stack((xyz, torch.ones(N, dtype=xyz.dtype, device=xyz.device)), dim=1)
        xyz_c = torch.matmul(c2w, homo_xyz.T).T
        homo_K = torch.eye(4, 4, dtype=xyz.dtype, device=xyz.device)
        
        xyz_p = torch.matmul(homo_K, xyz_c.T).T
        xyz_p[:, 0] /= xyz_p[:, 2]
        xyz_p[:, 1] /= xyz_p[:, 2]
        valid_flag = (0 <= xyz_p[:, 0] <= w) & (0 <= xyz_p[:, 1] <= h)
        
        return valid_flag

    # fuse position and feature
    def fusion(self, origin, to_update, valid_flag):

        # fuse position
        self.position_weight = 0.1
        origin_xyz = origin["xyz"]
        to_update_xyz = to_update["xyz"]
        origin_xyz[valid_flag] = origin_xyz[valid_flag] * (1 - self.position_weight) + to_update_xyz[valid_flag] * self.position_weight

        # fuse feature
        # TODO: create a fusion net
        self.feat_weight = 0.05
        origin_feat = origin["feat"]
        to_updte_feat = to_update["feat"]
        origin_feat[valid_flag] = origin_feat[valid_flag] * (1 - self.feat_weight) + to_updte_feat[valid_flag] * self.feat_weight

        origin.update("xyz", origin_xyz)
        origin.update("feat", origin_feat)

        return origin


    # view fusion
    def view_fusion(self, inputs, outputs):
        inv_K = rearrange(inv_K[:,None,...].
                          repeat(1, self.cfg.model.gaussians_per_pixel, 1, 1),
                          'b n ... -> (b n) ...')
