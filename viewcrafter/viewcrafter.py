...
class ViewCrafter:
    ...
    def nvs_single_view(self, gradio=False):
        ...
        _, H, W, _ = render_results.shape

        '''
        render_results = []
        transform = transforms.Compose([
            transforms.Resize((576, 1024)),  # 可以根据需要调整大小
            transforms.ToTensor(),
        ])
        folder='/home/zsj/Video2Scene/re10k_data'
        for filename in sorted(os.listdir(folder),key=lambda x:int(x.split('.')[0]))[:25]:
            img_path = os.path.join(folder, filename)
            if os.path.isfile(img_path):
                try:
                    img = Image.open(img_path).convert('RGB')  # 转换为RGB模式
                    img_tensor = transform(img)  # 应用转换
                    render_results.append(img_tensor)
                except Exception as e:
                    print(f"无法读取图片 {img_path}: {e}")
        

        # 将列表转换为一个Tensor，形状为 (B, C, H, W)
        render_results = torch.stack(render_results) if render_results else torch.empty(0)
        render_results = render_results.permute(0, 2, 3, 1)
        '''
        
        video_path='/home/zsj/Video2Scene/SceneGeneration/results/pigeon/flash3d_video.mp4'
        ...
        # 将列表转换为一个 4D tensor
        # B: batch size (帧数), H: height, W: width, C: channels
        render_results = torch.stack(frames)[0:25]
        print(render_results.shape)
